...
def counter(seconds,var , button):
    button.confing(state=tk.DISABLED)
    while seconds:
        sleep(1)
        seconds -= 1
        var.set(time_format(seconds))
    button.confing(state=tk.ACTIVE)
def start(number):
    if number == 1:
        ...
        th1.start()
    if number == 2:
        seconds2 = int(h_p_2.get()) * 3600 + int(m_p_2.get()) * 60 + int(s_p_3.get())
        th2 = Thread(target=counter, args=(seconds2,t2 , b2))
        th2.start()
    if number == 3:
        seconds3 = int(h_p_1.get()) * 3600 + int(m_p_3.get()) * 60 + int(s_p_3.get())
